...
class DoublyLinkedList<T extends Comparable<T>>
{
	...
	public void insertBefore( int pos, T data ) 
	{
		...
		else
		{
			...
			DLLNode leftNode = temp.prev;
			DLLNode rightNode= temp;
			DLLNode newNode = new DLLNode(data, leftNode, rightNode);
	...
	public T get(int pos) 
	{
		...
		if(pos>=size()|| pos<0 || isEmpty()==true)
		{
			return null;
		}
	...
	public boolean deleteAt(int pos) 
	{
		...
		else if (size()==2)
		{
			if(pos==0)
			{
				head=head.next; //doesnt need to set anything to null as new heads next value is already null
				                //what about new heads previous? still points to oldhead?
//				head.prev=null;
			}
			else
			{
	...
	public void push(T item) 
	{
		...
		else
		{
			...
			head = new DLLNode(item, null, prevHead);
			head.next =prevHead;
			prevHead.prev=head;
	...
	public void enqueue(T item) 
	{
		...
		if(tail!=null)
		{
			DLLNode prevTail= tail;
			tail = new DLLNode(item, prevTail, null);
			prevTail.next=tail;
